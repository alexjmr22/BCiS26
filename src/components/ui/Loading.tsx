...
const Loading: React.FC = () => {
    ...
    return (
        ...
        >
            ...
            {/* Modal Content */}
            <div
                className={`relative overflow-hidden bg-[#0c061a]/98 border border-white/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 max-w-[90%] md:max-w-lg w-full shadow-2xl transform transition-all duration-500 ease-out ${isOpen ? 'scale-100 opacity-100 animate-pulse-scale' : 'scale-95 opacity-0'
                    }`}
            >
                {/* Close Button */}
                <button
                    onClick={onClose}
                    className="absolute top-6 right-6 p-2 text-white/30 hover:text-white transition-colors cursor-pointer z-10"
                >
                    <MdClose size={24} />
                </button>
                ...
                <div className="flex flex-col items-center text-center space-y-4 md:space-y-6 relative z-10">
                    ...
                    <button
                        onClick={onClose}
                        className="w-full py-4 bg-brand-purple hover:bg-brand-purple/90 text-white font-bold font-orbitron rounded-2xl transition-all active:scale-95 shadow-lg shadow-brand-purple/20 cursor-pointer"
                    >
                        GOT IT
                    </button>
                </div>
